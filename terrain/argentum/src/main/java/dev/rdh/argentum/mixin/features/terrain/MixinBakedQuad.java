...
public abstract class MixinBakedQuad implements BakedQuadView {
    ...
    public float getX(int index) {
        return Float.intBitsToFloat(this.vertexData[index * 7]);
    }

    @Override
    public float getY(int index) {
        return Float.intBitsToFloat(this.vertexData[index * 7 + 1]);
    }

    @Override
    public float getZ(int index) {
        return Float.intBitsToFloat(this.vertexData[index * 7 + 2]);
    }

    @Override
    public int getColor(int index) {
        return this.vertexData[index * 7 + 3];
    }

    @Override
    public float getTexU(int index) {
        return Float.intBitsToFloat(this.vertexData[index * 7 + 4]);
    }

    @Override
    public float getTexV(int index) {
        return Float.intBitsToFloat(this.vertexData[index * 7 + 5]);
    }

    @Override
    public int getLight(int index) {
        return this.vertexData[index * 7 + 6];
    }
    ...
    public ModelQuadFacing getLightFace() {
        return PrimitiveModelUtil.fromDirection(this.direction);
    }
