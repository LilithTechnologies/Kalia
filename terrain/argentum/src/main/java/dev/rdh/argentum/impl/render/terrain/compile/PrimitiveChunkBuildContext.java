...
import dev.rdh.argentum.impl.extensions.SpriteAtlasTextureExtension;
...
public class PrimitiveChunkBuildContext extends ChunkBuildContext {
    ...
    public BufferBuilder getBuffer(RenderLayer layer) {
        ...
        if (!this.usedLayerBuffers[index]) {
            buffer.begin(7 /*GL_QUADS*/, VertexFormats.BLOCK);
    ...
    public void finishSection(ChunkBuildBuffers buffers) {
        for (int i = 0; i < this.layerBuffers.length; i++) {
            ...
            try {
                this.copyRawBuffer(buffer.getByteBuffer().asIntBuffer(), buffer.getVertexCount(), buffers,
    ...
    private static int getInitialBufferSize(RenderLayer layer) {
		return switch(layer) {
    ...
    private void outputQuads(IntBuffer rawBuffer, ChunkVertexEncoder.Vertex[] celeritasVertices, int numQuads, ChunkBuildBuffers buffers, Material material, int[] winding) {
        ...
        for (int quadIdx = 0; quadIdx < numQuads; quadIdx++) {
            ...
            Sprite sprite = this.textureAtlas.celeritas$findFromUV(uSum * 0.25F, vSum * 0.25F);
            if (sprite != null && sprite.hasMeta()
    ...
    public Material selectMaterial(Material material, Sprite sprite) {
        if (sprite == null || sprite.getClass() != Sprite.class || sprite.hasMeta()
