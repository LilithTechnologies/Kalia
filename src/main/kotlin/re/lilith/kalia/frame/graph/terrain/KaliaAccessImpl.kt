...
class KaliaAccessImpl : KaliaAccess {
    ...
    override fun gbufferEnabled(): Boolean = RayTracingGraph.isDeferred(KaliaEngine.device)

    override fun worldColorFormats(): List<TextureFormat> =
        if (gbufferEnabled()) RayTracingGraph.GBUFFER_FORMATS else listOf(sceneColorFormat())
