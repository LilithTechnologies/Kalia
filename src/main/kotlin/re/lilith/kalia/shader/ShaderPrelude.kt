...
object ShaderPrelude {
    object Bindings {
        const val BASE_TEXTURE = 0
...
layout(binding = ${Bindings.SCENE_UNIFORMS}, std140) uniform KaliaScene {
...
"""
    ...
    //language=glsl
    const val LIGHTING_FUNCTION = """
vec3 kaliaApplyDiffuse(vec3 color, vec3 normal) {
    ...
    float diffuse = min(1.0, 0.4 + 0.6 * (first + second));
    return color * diffuse;
}
"""

    //language=glsl
    const val OVERLAY_FUNCTION = """
vec3 kaliaApplyOverlay(vec3 color) {
    return mix(color, kaliaOverlayColor.rgb, kaliaOverlayColor.a);
}
"""
}
