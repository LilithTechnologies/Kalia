...
object ItemShaders {
    ...
    fun program(): ShaderProgram = cached ?: ShaderProgram(
        label = "kalia/item",
        stages = mapOf(
            ShaderStage.VERTEX to ShaderSource.Glsl("item.vert", ShaderAssets.assemble("item.vert", emptyList())),
            ShaderStage.FRAGMENT to ShaderSource.Glsl("item.frag", ShaderAssets.assemble("item.frag", emptyList())),
        ),
        bindings = listOf(
            ShaderBinding(
