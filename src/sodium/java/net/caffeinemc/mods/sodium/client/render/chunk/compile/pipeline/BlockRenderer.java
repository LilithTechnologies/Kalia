...
public class BlockRenderer {
    ...
    public void renderModel(BlockRenderContext ctx) {
        ...
        // Fast leaves (SOLID render layer) create a thin shell after self-occlusion.
        // Directional face group culling at the section level incorrectly hides shell faces
        // that are visible through the hollow interior, causing diagonal line artifacts
        // tied to camera perspective. Bypass directional culling by using UNASSIGNED.
        Block block = ctx.state().getBlock();
        boolean useUnassignedFaceGroup = block instanceof LeavesBlock && block.getRenderLayerType() == RenderLayer.SOLID;

        for (Direction face : DirectionUtil.ALL_DIRECTIONS) {
            List<BakedQuad> quads = this.getGeometry(ctx, face);

            if (!quads.isEmpty() && this.isFaceVisible(ctx, face)) {
                this.renderQuadList(ctx, material, lighter, colorizer, offset, meshBuilder, quads, face, useUnassignedFaceGroup);
            }
        }
        ...
        if (!all.isEmpty()) {
            this.renderQuadList(ctx, material, lighter, colorizer, offset, meshBuilder, all, null, useUnassignedFaceGroup);
        }
    }
    ...
    private void renderQuadList(BlockRenderContext ctx, Material material, LightPipeline lighter, ColorProvider colorizer, Vector3f offset,
                                ChunkModelBuilder builder, List<BakedQuad> quads, Direction cullFace, boolean useUnassignedFaceGroup) {
        ...
        for (int i = 0, quadsSize = quads.size(); i < quadsSize; i++) {
            ...
            this.writeGeometry(ctx, builder, offset, material, quad, vertexColors, lightData, useUnassignedFaceGroup);
    ...
    private void writeGeometry(BlockRenderContext ctx,
                               ...
                               int[] colors,
                               QuadLightData light,
                               boolean useUnassignedFaceGroup) {
        ModelQuadOrientation orientation = ModelQuadOrientation.orientByBrightness(light.br, light.lm);
        var vertices = this.vertices;

        ModelQuadFacing normalFace = useUnassignedFaceGroup ? ModelQuadFacing.UNASSIGNED : quad.getNormalFace();
